...
file_with_papers = os.path.join(data_dir, 'llm_evaluation.RetrievalExperiment_with_papers.json')
...
# Function to check if the relevant paper is found in the context source
def check_paper_in_context(query, data, relevant_paper):
    for item in data:
        if item['query'] == query:
            for source in item['context_source']:
...
for paper, queries in paper_query_mapping.items():
    for query in queries:
        with_paper_found = check_paper_in_context(query, data_with_papers, paper)
        without_paper_found = check_paper_in_context(query, data_without_papers, paper)
        results.append({
            'query': query,
            'paper_title': paper,
            'retrieved_with_paper': with_paper_found,
            'retrieved_without_paper': without_paper_found
        })
...
output_file = os.path.join(current_dir, 'data/comparison_results.csv')
